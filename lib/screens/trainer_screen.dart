                  ...
                  return BlocBuilder<TrainerBloc, TrainerState>(
                    builder: (contextTrainer, trainerState) {
                      if (searchState is SearchLoading ||
                          trainerState is TrainersLoading) {
                        return const Center(child: CircularProgressIndicator());
                      } else if (searchState is SearchUsersLoaded &&
                          trainerState is TrainersLoaded) {
                        final trainers = trainerState.trainers;
                        final usersToShow = searchState.users
                            .where((user) => !trainers.contains(user))
                            .toList();

                        // Verifica se existem duplicatas e as remove
                        final uniqueUsers = usersToShow.toSet().toList();
            ...
            child: BlocBuilder<TrainerBloc, TrainerState>(
              builder: (context, state) {
                if (state is TrainersLoading) {
                        ...
                        child: ListTile(
                          ...
                          trailing: IconButton(
                            ...
                            onPressed: () {
                              context
                                  .read<TrainerBloc>()
                                  .add(RemoveTrainer(user));
                              context.read<SearchBloc>().add(
                                  LoadUsersByFullName(_searchController.text));
                            },
                          ),
                        ),
                      );
                  ...
                  return Center(
                      child: Text('Nenhum treinador adicionado.',
                          style: Theme.of(context).textTheme.labelLarge));
                }
              },
            ),
