...
class PacoteBloc extends Bloc<PacoteEvent, PacoteState> {
  ...
  PacoteBloc(this.pacoteRepository, this.pacoteTreinoRepository) : super(PacoteInitial()) {
  ...
  Future<void> _onLoadPacoteById(LoadPacoteById event, Emitter<PacoteState> emit) async {
    emit(PacoteLoading());
    try {
      final pacote = await pacoteRepository.getPacoteById(event.pacoteId);
      final treinoIds = await pacoteTreinoRepository.getTreinoIdsByPacoteId(event.pacoteId);
